...
export function Skills() {
    return (
        <section id="skills" aria-labelledby="skills-heading" className="max-w-4xl mx-auto px-4 py-16 lg:py-24">
            <p className="text-xs font-normal tracking-widest text-mist-400 uppercase">04</p>
...
function SkillGroup({ group }: { group: { label: string; items: string; usedIn?: string } }) {
    return (
        <div className="py-6 lg:py-8">
            <dt className="text-xs font-medium tracking-widest text-mist-400 uppercase">{group.label}</dt>
            <dd className="mt-2 text-xl font-bold lg:text-2xl">{group.items}</dd>
            {group.usedIn && <p className="mt-2 text-xs text-mist-500">Used in {group.usedIn}</p>}
        </div>
    );
}
